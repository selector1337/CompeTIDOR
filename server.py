...
def account_record_keys(account):
    """Return every stable identifier used by older and newer persisted records."""
    values = (
        account.get("id"),
...
def account_record(records, account):
    records = records or {}
    for key in account_record_keys(account):
        if key in records:
            return records[key]
    ...
    return next(
        (
            ...
            if str(record.get("account") or "") == nickname
            or seller_id and str(record.get("seller_id") or "") == seller_id
        ),
        {},
    )
...
def build_operations(payload):
    ...
    for account in revenue_source_accounts:
        ...
        previous_available = previous_record.get("source") == "Pedidos oficiais Mercado Livre"
        revenue.append(
            {
                "account": account.get("nickname"),
                ...
                "previous_month_available": previous_available,
                "previous_sync_status": previous_record.get("sync_status") or "Sincronizando em segundo plano",
                "revenue_change_percent": percentage_change(current_amount, previous_amount) if previous_available else None,
                "orders_change_percent": percentage_change(current_orders, previous_orders) if previous_available else None,
    ...
    previous_pending_accounts = sum(1 for item in revenue if not item["previous_month_available"])
    previous_month_complete = bool(revenue) and previous_pending_accounts == 0
    ...
    return {
        ...
        "previous_month_complete": previous_month_complete,
...
class MercadoLivreClient:
    ...
    def listing_prices(
        self, site_id, price, listing_type_id, category_id="",
...
PRODUCT_IMPORT_HOSTS = {
    ...
    "bhphotovideo.com", "thomannmusic.com", "thomann.de", "guitarcenter.com",
}
...
def product_import_host(url):
    ...
    if parsed.scheme != "https" or not host:
        raise RuntimeError("Informe um link HTTPS válido do Mercado Livre, Amazon, B&H Photo, Thomann ou Guitar Center.")
...
def product_reader_download(url, maximum=None):
...
def product_page_download(url):
    ...
    try:
        ...
        # JavaScript. O leitor devolve o conteúdo público em Markdown, sem
        # executar scripts nem reutilizar cookies do visitante.
        try:
...
PRODUCT_TITLE_TRANSLATIONS = (
    ...
    (r"\bstandalone sampler and composer\b", "sampler e compositor autônomo"),
    (r"\bsampler and composer\b", "sampler e compositor"),
    (r"\bstandalone\b", "autônomo"),
    (r"\bbuilt-in effects\b", "efeitos integrados"),
    (r"\bbuilt-in\b", "integrado"),
    (r"\bsample memory\b", "memória de samples"),
    (r"\bstereo input\b", "entrada estéreo"),
    (r"\bstereo output\b", "saída estéreo"),
    (r"\banalog input\b", "entrada analógica"),
    (r"\banalog output\b", "saída analógica"),
    (r"\binput\b", "entrada"),
    (r"\boutput\b", "saída"),
    (r"\bincludes\b", "inclui"),
    (r"\bcompatible with\b", "compatível com"),
    (r"\bdesigned for\b", "desenvolvido para"),
    (r"\bup to\b", "até"),
    (r"\bchannels\b", "canais"),
    (r"\bchannel\b", "canal"),
...
def optimized_product_title(name, brand="", model=""):
    ...
    title = re.sub(r"^\s*(?:amazon(?:\.com(?:\.br)?)?|b&h(?: photo)?|thomann|guitar center)\s*[:|\-]\s*", "", title, flags=re.IGNORECASE)
    title = re.sub(r"\b(?:amazon exclusive|thomann exclusive|guitar center exclusive|visit the .+? store)\b", "", title, flags=re.IGNORECASE)
    title = re.split(r"\s+[|–—]\s+", title, maxsplit=1)[0]
    title = re.sub(r"\b(?:free shipping|free delivery|frete grátis|in stock|new)\b", "", title, flags=re.IGNORECASE)
    clean_title = re.sub(r"\s+", " ", title).strip(" -")
    clean_brand = clean_product_text(brand, 120)
    clean_model = clean_product_text(model, 120)
    parts = []
    if clean_brand and clean_brand.lower() not in clean_title.lower():
        parts.append(clean_brand)
    parts.append(clean_title)
    if clean_model and clean_model.lower() not in " ".join(parts).lower():
    ...
    clipped = re.sub(r"\s+(?:a|as|com|da|das|de|do|dos|e|em|para|por|sem)$", "", clipped, flags=re.IGNORECASE)
    return (clipped or title[:60]).strip()


def normalized_product_image_url(value, base_url=""):
...
def usable_product_image(value, base_url=""):
    url = normalized_product_image_url(value, base_url)
    parsed = urlparse(url)
    ...
    if parsed.hostname and (
        ...
        return ""
    known_image_service = any(domain in (parsed.hostname or "").lower() for domain in (
        "media.guitarcenter.com", "static.guitarcenter.com", "thumbs.static-thomann.de",
    )) or "/is/image/" in parsed.path.lower()
    if not known_image_service and not re.search(r"\.(?:jpe?g|png|webp)(?:$|\?)", url, flags=re.IGNORECASE):
        return ""
    return url
...
def product_image_urls(values, base_url=""):
    ...
    for value in values or []:
        ...
        if not raw:
            continue
        url = usable_product_image(raw, base_url)
        signature = ""
        if url:
            parsed = urlparse(url)
            signature = f"{parsed.netloc.lower()}{parsed.path.lower()}"
        existing_signatures = {f"{urlparse(row).netloc.lower()}{urlparse(row).path.lower()}" for row in rows}
        if url and signature not in existing_signatures:
            rows.append(url)
    return rows[:12]


def html_element_text(page_html, element_id):
    match = re.search(
        rf"<(?P<tag>[a-z0-9]+)[^>]*\bid=[\"']{re.escape(element_id)}[\"'][^>]*>(?P<body>.*?)</(?P=tag)>",
        page_html, flags=re.IGNORECASE | re.DOTALL,
    )
    return clean_product_text(match.group("body"), 12000) if match else ""


def amazon_product_page(page_html, page_url):
    title = html_element_text(page_html, "productTitle")
    if not title:
        return {}
    byline = html_element_text(page_html, "bylineInfo")
    brand = re.sub(r"^(?:visit the|brand:)\s+|\s+store$", "", byline, flags=re.IGNORECASE).strip()
    bullets_block = re.search(r"\bid=[\"']feature-bullets[\"'][^>]*>(.*?)</(?:div|ul)>", page_html, flags=re.IGNORECASE | re.DOTALL)
    bullets = []
    if bullets_block:
        for value in re.findall(r"<span[^>]*>(.*?)</span>", bullets_block.group(1), flags=re.IGNORECASE | re.DOTALL):
            value = clean_product_text(value, 1200)
    ...
    for label in ("Model Name", "Model Number", "Item model number"):
        ...
        if match:
            model = clean_product_text(match.group(1), 120)
            break
    if not brand:
...
def markdown_product_page(content, page_url):
    if not re.search(r"^(?:Title:|Markdown Content:|# )", content, flags=re.MULTILINE):
        return {}
    ...
    model = ""
    for row in specs:
        key = normalized_attribute_label(row["name"])
        if key in {"brand", "manufacturer", "marca", "fabricante"}:
            brand = row["value"]
...
def retailer_product_page(page_html, page_url, parser):
    ...
    raw_urls = [html.unescape(url.replace("\\/", "/")) for url in raw_urls]
    if "thomann" in host:
        raw_urls = [
            *[url for url in raw_urls if "static-thomann.de" in url or "/pics/bdb/" in url],
            *[url for url in parser.images if "static-thomann.de" in str(url) or "/pics/bdb/" in str(url)],
        ]
    elif "guitarcenter.com" in host:
        raw_urls = [
            *[url for url in raw_urls if "guitarcenter.com/is/image/" in url or "media.guitarcenter.com" in url],
            *[url for url in parser.images if "guitarcenter.com/is/image/" in str(url) or "media.guitarcenter.com" in str(url)],
        ]
    else:
        raw_urls = [
            *[url for url in raw_urls if "bhphoto.com/images/" in url or "static.bhphoto.com" in url],
            *[url for url in parser.images if "bhphoto.com" in str(url) or "/images/" in str(url)],
        ]
    specs = []
    pairs = re.findall(
        r"<(?:dt|th)[^>]*>(.*?)</(?:dt|th)>\s*<(?:dd|td)[^>]*>(.*?)</(?:dd|td)>",
        page_html, flags=re.IGNORECASE | re.DOTALL,
    )
    for label, value in pairs:
        label, value = clean_product_text(label, 100), clean_product_text(value, 400)
        if 1 < len(label) < 100 and value and len(value) < 400:
            specs.append({"name": label, "value": value})
    return {"brand": clean_product_text(brand, 120), "images": product_image_urls(raw_urls, page_url), "specifications": specs[:80]}
...
def parse_external_product_page(page_html, page_url):
    ...
    for row in specifications:
        key = normalized_attribute_label(row.get("name") or row.get("id"))
        if key in {"brand", "manufacturer", "marca", "fabricante"}:
            spec_identity.setdefault("brand", row.get("value"))
        elif key in {"model", "model number", "modelo"}:
            spec_identity.setdefault("model", row.get("value"))
        elif key in {"manufacturer part number", "part number", "mfr", "mpn"}:
            spec_identity.setdefault("mpn", row.get("value"))
    ...
    brand = brand or spec_identity.get("brand") or ""
    mpn = specific.get("mpn") or node.get("mpn") or spec_identity.get("mpn") or node.get("sku") or ""
    model = specific.get("model") or node.get("model") or spec_identity.get("model") or spec_identity.get("mpn") or node.get("mpn") or ""
    return {
        "source_url": page_url,
...
def imported_meli_product(payload, item_id, source_url):
    ...
    owner = next(
        (
            ...
            and str(account.get("seller_id") or "") == str(item.get("seller_id") or "")
        ),
        None,
    )
    if owner:
        try:
            bundle = clone_source_bundle(payload, owner.get("id") or owner.get("nickname"), item.get("id") or item_id, include_description=True)
            item = bundle.get("source_item") or item
            catalog_product = bundle.get("catalog_product") or {}
            description = bundle.get("description") or ""
        except Exception:
            pass
    if not catalog_product and item.get("catalog_product_id"):
        reader = official_reader_client(payload)
        if reader:
            try:
                catalog_product = call_interactive_client_method(reader.product, item.get("catalog_product_id")) or {}
            except Exception:
                catalog_product = {}
    try:
        if not description:
...
def generated_product_description(product):
    ...
    for row in product.get("specifications") or []:
        ...
        if len(seen) >= 30:
            break
    english_source = product.get("source_host", "").endswith(("amazon.com", "amazon.com.br", "bhphotovideo.com", "thomannmusic.com", "thomann.de", "guitarcenter.com"))
    if feature_lines:
        feature_text = "\n".join(feature_lines)
...
def imported_package_measurements(product):
    """Return real package measures found at the source, ready for numeric UI fields."""
    result = {}
    for row in product.get("raw_attributes") or []:
        attr_id = str(row.get("id") or "").upper()
        field = {
        ...
        value = clone_attribute_display_value(row)
        try:
            normalized = seller_package_api_value(field, value)
            result[field] = parse_decimal_number(normalized)
        except (RuntimeError, ValueError):
            pass
    for row in product.get("specifications") or []:
        label = normalized_attribute_label(row.get("name") or row.get("id"))
        value = clean_attribute_value(row.get("value"))
        if not value:
            continue
        if "package weight" in label or "peso da embalagem" in label:
            match = re.search(r"([\d.,]+)\s*(lb|lbs|oz|kg|g)\b", value, flags=re.IGNORECASE)
            if match:
                amount, unit = parse_decimal_number(match.group(1)), match.group(2).lower()
                grams = amount * {"lb": 453.59237, "lbs": 453.59237, "oz": 28.349523125, "kg": 1000, "g": 1}[unit]
                result.setdefault("package_weight", round(grams, 2))
        if "box dimensions" in label or "package dimensions" in label or "dimensoes da embalagem" in label:
            match = re.search(
                r"([\d.,]+)\s*[x×]\s*([\d.,]+)\s*[x×]\s*([\d.,]+)\s*(?:inches|inch|in|\"|cm)?",
                value, flags=re.IGNORECASE,
            )
            if match:
                length, width, height = (parse_decimal_number(part) for part in match.groups())
                is_inches = bool(re.search(r"(?:inches|inch|\bin\b|\")", value, flags=re.IGNORECASE))
                factor = 2.54 if is_inches else 1
...
def publication_attribute_rows(product, definitions):
    ...
    for definition in definitions or []:
        ...
        if not attr_id or not clone_attribute_user_editable(definition, attr_id):
            continue
        if not clone_attribute_applies_to_source(
            {"condition": product.get("condition") or "new"}, attr_id, definition.get("name") or ""
        ):
            continue
...
def import_product_operation(payload, request):
    ...
    if item_id:
        ...
        # A Amazon às vezes responde HTTP 200 com um shell sem dados do produto.
        # Nesse caso, force a leitura pública em Markdown antes de desistir.
        if "amazon." in host and not product.get("title"):
            try:
                page_html, final_url = product_reader_download(source_url)
                product = parse_external_product_page(page_html, final_url)
            except Exception:
                pass
...
def product_category_operation(payload, request):
    ...
    query = clean_product_text(request.get("query"), 240)
    if query and not category_id:
        if re.fullmatch(r"ML[A-Z]\d+", query.upper()):
            category_id = query.upper()
            try:
                category = reader.category(category_id) or {}
            except Exception as exc:
                raise RuntimeError("A categoria informada não existe ou não está disponível.") from exc
            return {"suggestions": [{
        ...
        try:
            suggestions = reader.domain_discovery(query, "MLB", 8) or []
        except Exception as exc:
            raise RuntimeError("Não foi possível consultar categorias agora.") from exc
...
def auto_official_sync_loop():
    ...
    while True:
        try:
            ...
            accounts = [account for account in official_accounts if not account_sync_is_active(account)]
            changed = False
            run_count += 1
            # Reputation is a small request. Refresh it before item/order workloads so the
            # dashboard never waits behind a large account synchronization.
            for account in official_accounts:
                try:
                    metric = next(
                        (row for row in payload.get("metrics", []) if row.get("account") == account.get("nickname")),
                        {},
                    )
                    ...
                    account["reputation_sync_error"] = str(exc)
                    changed = True
            if changed:
                write_payload(payload)
...
def sync_previous_month_revenue(payload, account, client):
    ...
    if (
        cached.get("source") == "Erro temporário"
        and failed_at
        and (datetime.now(APP_TZ) - failed_at).total_seconds() < 900
    ):
        return cached
    try:
        orders = fetch_seller_orders_window(client, account.get("seller_id"), date_from, date_to)
...
def assisted_publication_attributes(draft):
    ...
    if sku and not seen.intersection({"SELLER_SKU", "SKU"}):
        ...
        seen.add("SELLER_SKU")
    for field, attr_id, unit in (
        ("package_height", "SELLER_PACKAGE_HEIGHT", "cm"),
        ("package_width", "SELLER_PACKAGE_WIDTH", "cm"),
        ("package_length", "SELLER_PACKAGE_LENGTH", "cm"),
        ("package_weight", "SELLER_PACKAGE_WEIGHT", "g"),
    ):
...
def build_assisted_publication_payload(draft, variant):
    ...
    package_labels = {
        "package_weight": "peso", "package_height": "altura",
        "package_width": "largura", "package_length": "comprimento",
    }
    missing_package = []
    for field, label in package_labels.items():
        try:
            if parse_decimal_number(draft.get(field)) <= 0:
                missing_package.append(label)
        except (TypeError, ValueError):
            missing_package.append(label)
    if missing_package:
        raise RuntimeError(
            "Informe as medidas reais da embalagem antes de publicar: " + ", ".join(missing_package) + "."
        )
...
def optional_money(value):
    if isinstance(value, dict):
        value = first_present(value, ["amount", "value", "cost"], None)
    try:
...
def order_item_fee_amount(order_item, catalog_item, line_total, quantity=1):
    ...
    per_unit_candidates = (order_item.get("sale_fee"), order_item.get("sale_fee_amount"))
    for candidate in per_unit_candidates:
        amount = optional_money(candidate)
        if amount is not None:
            return round(max(0.0, amount) * max(1, int(quantity or 1)), 2)
    nested_total = optional_money(first_present(order_item, ["sale_fee_details.total", "fees.sale_fee"], None))
    if nested_total is not None:
        return round(max(0.0, nested_total), 2)
...
def shipment_financials(client, order, seller_id, is_flex, catalog_item, local_cache, sale_amount=None, unit_amounts=None):
    ...
    try:
        ...
        if is_flex is True:
            ...
            estimate = optional_money(catalog_item.get("shipping_cost"))
...
def query_sales_report(payload, request):
    ...
    for account in accounts:
        ...
        try:
            orders, account_truncated = fetch_statistics_orders(
                client, account.get("seller_id"), start, end
            )
            truncated = truncated or account_truncated
        except Exception as exc:
            warnings.append(f"{account.get('nickname')}: {policy_error_message(exc, 'a leitura das vendas do período')}")
                ...
                rows.append(
                    {
                        ...
                        "sale_fee_amount": fee,
                        "percentage_fee_amount": percentage_fee,
    ...
    return {
        ...
        "summary": {
            ...
            "percentage_fee_amount": round(sum(float(row.get("percentage_fee_amount") or 0) for row in rows), 2),
...
def query_brand_sales_report(payload, request):
    ...
    for item in matching_items:
        ...
        row = grouped.setdefault(
            sku,
            {
                ...
                "gross_amount": 0.0,
                "sale_fee_amount": 0.0,
                "percentage_fee_amount": 0.0,
                "fixed_fee_amount": 0.0,
                "shipping_amount": 0.0,
                "shipping_reimbursement_amount": 0.0,
                "net_amount": 0.0,
                "cost_amount": 0.0,
                "profit_amount": 0.0,
                "known_fee_sales": 0,
    ...
    for sale in sales_rows:
        ...
        row["shipping_reimbursement_amount"] += float(sale.get("shipping_reimbursement_amount") or 0)
        ...
        if sale.get("sale_fee_amount") is not None:
            ...
            row["percentage_fee_amount"] += float(sale.get("percentage_fee_amount") or 0)
            row["fixed_fee_amount"] += float(sale.get("fixed_fee_amount") or 0)
        ...
        output.append(
            {
                **{key: value for key, value in row.items() if not isinstance(value, set)},
                ...
                "percentage_fee_amount": round(row["percentage_fee_amount"], 2) if all_fees_known and sales_count else 0.0 if not sales_count else None,
                "fixed_fee_amount": round(row["fixed_fee_amount"], 2) if all_fees_known and sales_count else 0.0 if not sales_count else None,
                "shipping_amount": round(row["shipping_amount"], 2),
                "shipping_reimbursement_amount": round(row["shipping_reimbursement_amount"], 2),
...
def bounded_score(value, minimum=0.0, maximum=100.0):
    try:
...
def purchase_price_range(value):
    ...
    if price <= 100:
        return "R$ 50 a R$ 100"
    if price <= 250:
        return "R$ 100 a R$ 250"
    if price <= 500:
        return "R$ 250 a R$ 500"
    if price <= 1000:
...
def query_purchase_intelligence(payload, request):
    ...
    if abc_basis not in {"profit", "net", "gross", "units"}:
        abc_basis = "profit"
    try:
        lead_time_days = max(1, min(365, int(float((request or {}).get("lead_time_days") or 15))))
    except (TypeError, ValueError):
        lead_time_days = 15
    try:
        review_days = max(1, min(365, int(float((request or {}).get("review_days") or 15))))
    except (TypeError, ValueError):
        review_days = 15
    try:
        service_level = max(80.0, min(99.9, float((request or {}).get("service_level") or 95)))
    except (TypeError, ValueError):
        service_level = 95.0
    try:
        desired_margin = max(0.0, min(90.0, float((request or {}).get("desired_margin") or 20)))
    except (TypeError, ValueError):
        desired_margin = 20.0
    service_z = 2.33 if service_level >= 99 else 1.88 if service_level >= 97 else 1.65 if service_level >= 95 else 1.28
    ...
    if account_filter != "all":
        accounts = [
            ...
            if account.get("id") == account_filter
            or str(account.get("seller_id") or "") == account_filter
            or str(account.get("nickname") or "") == account_filter
        ]
    if not accounts:
        raise RuntimeError("Nenhuma conta oficial conectada corresponde ao filtro selecionado.")
    allowed_ids = {
    ...
    for item in payload.get("catalog") or []:
        ...
        if item.get("account"):
            row["accounts"].add(str(item.get("account")))
        if item.get("id"):
            row["item_ids"].add(str(item.get("id")))
        if item_status == "active":
            row["active_listings"] += 1
            ...
            row["paused_listings"] += 1
        if is_catalog_listing(item):
            row["catalog_listings"] += 1
        else:
            row["traditional_listings"] += 1
        competition_status = str(item.get("status") or "").lower()
        if competition_status == "winning":
            row["winning_listings"] += 1
        elif competition_status == "sharing":
            row["sharing_listings"] += 1
        elif competition_status == "losing":
            row["losing_listings"] += 1
        try:
            stock = max(0, int(float(item.get("stock") or 0)))
        except (TypeError, ValueError):
            stock = 0
    ...
    if not grouped:
        raise RuntimeError("Nenhum SKU corresponde às contas e ao status selecionados.")
    for sku, stock in stock_by_sku.items():
        grouped[sku]["current_stock"] = stock
    ...
    for sale in sales.get("rows") or []:
        ...
        if not row:
            continue
        quantity = max(0, int(sale.get("quantity") or 0))
        row["sales"] += 1
        row["units"] += quantity
        row["gross_amount"] += float(sale.get("gross_amount") or 0)
        row["shipping_amount"] += float(sale.get("shipping_amount") or 0)
        ...
        if sale.get("profit_amount") is not None:
            ...
            row["profit_amount"] += float(sale.get("profit_amount") or 0)
        sold_at = parse_meli_datetime(sale.get("date"))
        day_key = sold_at.astimezone(APP_TZ).date().isoformat() if sold_at else str(sale.get("date") or "")[:10]
        if day_key:
            row["daily_units"][day_key] = row["daily_units"].get(day_key, 0) + quantity
        sold_label = sold_at.isoformat() if sold_at else str(sale.get("date") or "")
        if sold_label > str(row.get("last_sale_at") or ""):
            row["last_sale_at"] = sold_label
        if not row.get("thumbnail") and sale.get("thumbnail"):
            row["thumbnail"] = sale.get("thumbnail")
    ...
    for row in grouped.values():
        ...
        sales_count = int(row.get("sales") or 0)
        stock = int(row.get("current_stock") or 0)
        daily_average = units / period_days if units else 0.0
        ...
        suggested_reorder = max(0, target_stock - stock)
        coverage_days = stock / daily_average if daily_average > 0 else None
        ...
        all_profit_known = sales_count == row["known_profit_sales"] if sales_count else unit_cost is not None
        gross_amount = round(row["gross_amount"], 2)
        fee_amount = round(row["sale_fee_amount"], 2) if all_fees_known and sales_count else 0.0 if not sales_count else None
        net_amount = round(row["net_amount"], 2) if all_net_known and sales_count else 0.0 if not sales_count else None
        cost_amount = round(row["cost_amount"], 2) if all_costs_known and sales_count else 0.0 if not sales_count and unit_cost is not None else None
        profit_amount = round(row["profit_amount"], 2) if all_profit_known and sales_count else 0.0 if not sales_count and unit_cost is not None else None
        profit_margin = round((profit_amount / net_amount) * 100, 2) if profit_amount is not None and net_amount and net_amount > 0 else None
        monthly_roi = (
            round((profit_amount / cost_amount) * (30 / period_days) * 100, 2)
            if profit_amount is not None and cost_amount and cost_amount > 0 else None
        )
        average_price = round(gross_amount / units, 2) if units else 0.0
        average_net_unit = round(net_amount / units, 2) if net_amount is not None and units else None
        ...
        competition_total = row["winning_listings"] + row["sharing_listings"] + row["losing_listings"]
        competition_score = (
            (row["winning_listings"] * 100 + row["sharing_listings"] * 60 + row["losing_listings"] * 25) / competition_total
            if competition_total else 50.0
        )
        data_quality = 100.0 if not sales_count else (
            50 * row["known_fee_sales"] / sales_count + 50 * row["known_profit_sales"] / sales_count
        )
        if not units:
            stock_status = "Sem giro no período"
        elif stock <= reorder_point:
            stock_status = "Ruptura iminente"
        elif coverage_days is not None and coverage_days < lead_time_days + review_days:
            stock_status = "Repor em breve"
        elif coverage_days is not None and coverage_days > (lead_time_days + review_days) * 3:
            stock_status = "Excesso de estoque"
        else:
            stock_status = "Cobertura adequada"
        abc_value = {
        ...
        output.append(
            {
                **{key: value for key, value in row.items() if key not in {"accounts", "item_ids", "orders", "daily_units"}},
                "accounts": sorted(row["accounts"]),
                "accounts_label": ", ".join(sorted(row["accounts"])),
                "item_ids": sorted(row["item_ids"]),
                "orders": len(row["orders"]),
                ...
                "price_range": purchase_price_range(average_price),
                "competition_score": round(competition_score, 1),
                "data_quality": round(data_quality, 1),
                "abc_value": max(0.0, float(abc_value or 0)),
            }
        )

    abc_basis_used = abc_basis
    abc_total = sum(float(row.get("abc_value") or 0) for row in output)
    if abc_total <= 0:
        abc_basis_used = "units"
        for row in output:
            row["abc_value"] = float(row.get("units") or 0)
        abc_total = sum(float(row.get("abc_value") or 0) for row in output)
    ranked_rows = sorted(output, key=lambda current: (-float(current.get("abc_value") or 0), str(current.get("sku") or "")))
    if abc_total <= 0:
        for row in ranked_rows:
            row["abc_class"] = "C"
            row["abc_cumulative_percentage"] = 100.0
    else:
        abc_running = 0.0
        for index, row in enumerate(ranked_rows):
            abc_running += float(row.get("abc_value") or 0)
            cumulative = abc_running / abc_total * 100
            row["abc_class"] = "A" if cumulative <= 80 or index == 0 else "B" if cumulative <= 95 else "C"
            row["abc_cumulative_percentage"] = round(cumulative, 2)

    maximum_demand = max((float(row.get("average_daily_units") or 0) for row in output), default=0.0)
    for row in output:
        demand_ratio = float(row.get("average_daily_units") or 0) / maximum_demand if maximum_demand > 0 else 0.0
        demand_score = 100 * math.sqrt(demand_ratio) if demand_ratio > 0 else 0.0
        margin_score = 40.0 if row.get("profit_percentage") is None else (float(row["profit_percentage"]) + 10) / 40 * 100
        capital_score = 40.0 if row.get("monthly_roi") is None else (float(row["monthly_roi"]) + 10) / 50 * 100
        stability_score = {"X": 100.0, "Y": 65.0, "Z": 30.0}.get(row.get("xyz_class"), 30.0)
        score = (
            bounded_score(demand_score) * 0.25
            + bounded_score(margin_score) * 0.20
            + bounded_score(capital_score) * 0.20
            + bounded_score(row.get("competition_score")) * 0.15
            + stability_score * 0.10
            + bounded_score(row.get("data_quality")) * 0.10
        )
        row["attractiveness_score"] = bounded_score(score)
        row["decision"] = (
            "Compra prioritária" if score >= 80
            else "Comprar" if score >= 65
            else "Negociar condições" if score >= 50
            else "Compra cautelosa" if score >= 35
            else "Não recomendado"
        )
        confidence_score = (
            min(100.0, float(row.get("units") or 0) / 20 * 100) * 0.5
            + min(100.0, period_days / 90 * 100) * 0.2
            + bounded_score(row.get("data_quality")) * 0.3
        )
        row["confidence_score"] = bounded_score(confidence_score)
        row["confidence"] = "Alta" if confidence_score >= 75 else "Média" if confidence_score >= 45 else "Baixa"

    output.sort(key=lambda row: (-float(row.get("attractiveness_score") or 0), -int(row.get("units") or 0), str(row.get("sku") or "")))
    costed = [row for row in output if row.get("profit_amount") is not None]
    total_net = sum(float(row.get("net_amount") or 0) for row in costed)
    total_profit = sum(float(row.get("profit_amount") or 0) for row in costed)
    return {
        "ok": True,
        "kind": "purchase_intelligence",
        "date_from": start.isoformat(),
        "date_to": end.isoformat(),
        ...
        "summary": {
            ...
            "profit_percentage": round(total_profit / total_net * 100, 2) if total_net > 0 else None,
            "suggested_reorder": sum(int(row.get("suggested_reorder") or 0) for row in output),
            "capital_required": round(sum(float(row.get("suggested_reorder") or 0) * float(row.get("unit_cost") or 0) for row in output), 2),
            "abc": {letter: sum(1 for row in output if row.get("abc_class") == letter) for letter in ("A", "B", "C")},
            "xyz": {letter: sum(1 for row in output if row.get("xyz_class") == letter) for letter in ("X", "Y", "Z")},
            "urgent": sum(1 for row in output if row.get("stock_status") == "Ruptura iminente"),
            "high_confidence": sum(1 for row in output if row.get("confidence") == "Alta"),
        },
        "warnings": sales.get("warnings") or [],
        "truncated": bool(sales.get("truncated")),
        "methodology": {
...
def report_dataset(payload, report_type, filters, statistics_result=None):
    ...
    if report_type == "purchases":
        ...
        columns = [
            ("attractiveness_score", "Índice de atratividade", "number"),
            ("decision", "Decisão sugerida", "text"),
            ("confidence", "Confiança", "text"),
            ("abc_class", "Curva ABC", "text"),
            ("xyz_class", "Curva XYZ", "text"),
            ("sku", "SKU", "text"),
            ("product", "Produto", "text"),
            ("brand", "Marca", "text"),
            ("accounts_label", "Contas", "text"),
            ("current_stock", "Estoque físico consolidado", "integer"),
            ("units", "Unidades vendidas", "integer"),
            ("orders", "Pedidos", "integer"),
            ("average_daily_units", "Demanda média diária", "number"),
            ...
            ("monthly_roi", "Retorno mensal do capital (%)", "percent"),
            ("average_unit_price", "Preço médio de venda", "currency"),
            ("maximum_purchase_cost", "Custo máximo de compra", "currency"),
            ("coverage_days", "Cobertura atual (dias)", "number"),
            ("safety_stock", "Estoque de segurança", "integer"),
            ("reorder_point", "Ponto de reposição", "integer"),
            ("target_stock", "Estoque alvo", "integer"),
            ("suggested_reorder", "Compra sugerida", "integer"),
            ("stock_status", "Situação do estoque", "text"),
            ("days_since_last_sale", "Dias sem venda", "integer"),
            ("data_quality", "Qualidade dos dados (%)", "percent"),
        ]
        return (
            "Inteligência de compras e abastecimento",
            columns,
            result.get("rows") or [],
            {
                "Período": f"{result.get('date_from')} a {result.get('date_to')}",
                "Conta": filters.get("account") or "Todas",
                "Status ML": filters.get("ml_status") or "Ativos",
                "Base da curva ABC": result.get("abc_basis") or "profit",
                "Prazo de reposição": f"{result.get('lead_time_days') or 15} dias",
                "Ciclo de revisão": f"{result.get('review_days') or 15} dias",
                "Nível de serviço": f"{result.get('service_level') or 95}%",
                "Margem desejada": f"{result.get('desired_margin') or 20}%",
                "Gerado em": now_label(),
            },
        )
    if report_type == "brand_sales":
        ...
        columns = [
            ...
            ("percentage_fee_amount", "Tarifas de venda", "currency"),
            ("fixed_fee_amount", "Custos fixos", "currency"),
            ("shipping_amount", "Fretes debitados", "currency"),
            ("shipping_reimbursement_amount", "Estornos Flex", "currency"),
            ("net_amount", "Valor líquido", "currency"),
