...
document.addEventListener("error", (event) => {
  ...
  if (!(image instanceof HTMLImageElement) || !image.matches(".sale-thumb")) return;
  const fallback = document.createElement("span");
  fallback.className = "sale-thumb sale-thumb-empty";
  fallback.setAttribute("aria-label", "Imagem indisponível");
...
function analyticsSeriesChart(current, comparison, field, currency = false) {
  ...
  const currentByIndex = new Map(currentRows.map((row) => [Number(row.index), row]));
  const comparisonByIndex = new Map(comparisonRows.map((row) => [Number(row.index), row]));
  const hoverColumns = Array.from({ length: count }, (_, offset) => {
    const index = offset + 1;
    const currentRow = currentByIndex.get(index);
    const comparisonRow = comparisonByIndex.get(index);
    const centerX = x(index);
    const previousX = index === 1 ? left : (x(index - 1) + centerX) / 2;
    const nextX = index === count ? width - right : (centerX + x(index + 1)) / 2;
    const currentY = currentRow ? y(currentRow[field]) : null;
    const comparisonY = comparisonRow ? y(comparisonRow[field]) : null;
    const tooltipWidth = 238;
    const tooltipHeight = 102;
    const tooltipX = Math.max(left, Math.min(width - right - tooltipWidth, centerX - tooltipWidth / 2));
    const highestPoint = Math.min(...[currentY, comparisonY].filter((value) => value !== null));
    const tooltipY = Math.max(4, Math.min(top + plotHeight - tooltipHeight - 8, highestPoint - tooltipHeight - 12));
    const currentLabel = current?.label || "Período atual";
    const comparisonLabel = comparison?.label || "Período anterior";
    const aria = `Dia ${index}. ${currentLabel}: ${currentRow ? exactValue(currentRow[field]) : "sem dado"}. ${comparisonLabel}: ${comparisonRow ? exactValue(comparisonRow[field]) : "sem dado"}.`;
    const tooltipRow = (row, tone, label) => `<div class="analytics-tooltip-row"><i class="${tone}"></i><span><b>${escapeText(label)}</b><small>${row ? analyticsDateLabel(row.date) : "Sem data correspondente"}</small></span><strong>${row ? escapeText(exactValue(row[field])) : "—"}</strong></div>`;
    return `<g class="analytics-hover-group" tabindex="0" aria-label="${escapeAttr(aria)}">
      <rect x="${previousX.toFixed(1)}" y="${top}" width="${Math.max(1, nextX - previousX).toFixed(1)}" height="${plotHeight}" class="analytics-hover-target"/>
      <line x1="${centerX.toFixed(1)}" y1="${top}" x2="${centerX.toFixed(1)}" y2="${top + plotHeight}" class="analytics-hover-guide"/>
      ${comparisonRow ? `<circle cx="${centerX.toFixed(1)}" cy="${comparisonY.toFixed(1)}" r="5" class="analytics-hover-marker comparison"/>` : ""}
      ${currentRow ? `<circle cx="${centerX.toFixed(1)}" cy="${currentY.toFixed(1)}" r="5" class="analytics-hover-marker current"/>` : ""}
      <foreignObject x="${tooltipX.toFixed(1)}" y="${tooltipY.toFixed(1)}" width="${tooltipWidth}" height="${tooltipHeight}" class="analytics-floating-tooltip"><div xmlns="http://www.w3.org/1999/xhtml"><em>Dia ${index}</em>${tooltipRow(comparisonRow, "comparison", comparisonLabel)}${tooltipRow(currentRow, "current", currentLabel)}</div></foreignObject>
    </g>`;
  }).join("");
...
function analyticsDonut(entries, valueField, shareField, total, currency = false) {
  ...
  return `<div class="analytics-donut-block"><div class="analytics-donut-layout">
...
function renderCustomers() {
  ...
  if (!data) return;
  const syncAccounts = Object.values(data.sync?.accounts || {});
  const missingDocuments = syncAccounts.reduce((sum, row) => sum + Number(row.missing_document || 0), 0);
  ...
  if (!state.customersLoading && !state.customersError && (missingDocuments || missingLocations)) {
    const issueNames = {
      ...
      "billing:resposta_sem_documento": "resposta fiscal sem CPF/CNPJ",
      "billing:resposta_sem_endereco_fiscal": "resposta fiscal sem endereço",
      "billing:acesso_negado": "acesso fiscal negado pela credencial",
      "billing:credencial_sem_autorizacao": "credencial fiscal sem autorização",
      "billing:nao_encontrado": "dados fiscais não encontrados",
    ...
    const reasons = Object.entries(issueTotals).sort((a, b) => b[1] - a[1]).slice(0, 3).map(([key, value]) => `${issueNames[key] || key.replaceAll("_", " ")} (${value})`).join("; ");
    feedback.innerHTML = `<div class="notice danger-notice"><strong>Importação parcial:</strong> ${missingDocuments.toLocaleString("pt-BR")} pedido(s) sem documento e ${missingLocations.toLocaleString("pt-BR")} sem endereço fiscal/de entrega completo.${reasons ? ` Motivos identificados: ${escapeText(reasons)}.` : ""} Execute novamente a importação; falhas temporárias serão reconsultadas.${reconciliation ? `<br><small>${reconciliation}</small>` : ""}</div>`;
  } else if (!state.customersLoading && !state.customersError && reconciliation) {
    feedback.innerHTML = `<div class="notice"><strong>Última conciliação com o Mercado Livre</strong><br><small>${reconciliation}</small></div>`;
  }
  ...
  summary.innerHTML = `
    ...
    <article><span>Prontos para NF</span><strong>${Number(totals.invoice_ready || 0).toLocaleString("pt-BR")}</strong><small>Com CPF/CNPJ e endereço fiscal</small></article>`;
...
async function syncCustomers() {
  ...
  try {
    ...
    const found = Number(result.orders_seen || 0);
...
function updateBulkPriceBar() {
  const root = document.querySelector("#ads-bulk-price");
  if (!root) return;
...
function metricLine(label, value, max, inverse) {
  ...
  const percent = Math.min(100, Math.max(0, Math.round((numeric / max) * 100)));
  const color = inverse
    ? percent > 60
      ? "#dc2626"
      : percent > 35
  ...
  return `
    <div class="metric-line">
      ...
      <strong>${numeric.toLocaleString("pt-BR", { minimumFractionDigits: 0, maximumFractionDigits: 2 })}%</strong>
    </div>
  `;
}
...
async function downloadBulkSpreadsheet() {
  ...
  try {
    ...
    if (!response.ok) throw new Error(initial.error || "Não foi possível gerar a planilha.");
    let job = initial;
    while (!["completed", "error"].includes(job.status)) {
      ...
      job = await poll.json();
      if (!poll.ok) throw new Error(job.error || "Não foi possível acompanhar a planilha.");
    }
    if (job.status === "error") throw new Error(job.message || "Não foi possível gerar a planilha.");
    state.spreadsheetProgress = { progress: 100, message: "Planilha pronta para baixar." };
    renderSpreadsheetPreview();
    const downloadResponse = await fetch(job.download_url, { credentials: "same-origin" });
    if (!downloadResponse.ok) throw new Error("Não foi possível baixar a planilha concluída.");
    const blob = await downloadResponse.blob();
    const disposition = downloadResponse.headers.get("Content-Disposition") || "";
    const filename = disposition.match(/filename="?([^";]+)"?/i)?.[1] || job.filename || "competidor-edicao-anuncios.xlsx";
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = filename;
...
function renderSpreadsheetPreview() {
  ...
  if (state.spreadsheetProgress) {
    const progress = Math.max(0, Math.min(100, Number(state.spreadsheetProgress.progress || 0)));
    root.innerHTML = `<section class="spreadsheet-review spreadsheet-progress"><div class="spreadsheet-review-head"><div><small>Processamento em segundo plano</small><strong>${escapeText(state.spreadsheetProgress.message || "Processando planilha...")}</strong></div><span>${progress}%</span></div><div class="job-progress-track"><span style="width:${progress}%"></span></div><small>Você pode continuar usando a aplicação enquanto este trabalho é concluído.</small></section>`;
    return;
  }
  if (!preview) {
    root.innerHTML = "";
...
async function importBulkSpreadsheet(file) {
  ...
  try {
    ...
    const queued = await api("/api/spreadsheet/import", {
      method: "POST",
      body: JSON.stringify({ file: await fileAsBase64(file) }),
    });
    state.spreadsheetProgress = { progress: 15, message: "Validando a planilha em segundo plano." };
    renderSpreadsheetPreview();
    const result = await waitForAsyncOperation(queued, (message) => {
      state.spreadsheetProgress = { progress: 60, message: message || "Validando linhas da planilha." };
      renderSpreadsheetPreview();
    });
    state.spreadsheetProgress = null;
...
async function applyBulkSpreadsheet(button) {
  ...
  try {
    const queued = await api("/api/spreadsheet/apply", {
      method: "POST",
      body: JSON.stringify({ token: preview.token }),
    });
    state.spreadsheetProgress = { progress: 10, message: "Aplicando alterações em segundo plano." };
    renderSpreadsheetPreview();
    const result = await waitForAsyncOperation(queued, (message) => {
      state.spreadsheetProgress = { progress: 55, message: message || "Atualizando anúncios no Mercado Livre." };
      renderSpreadsheetPreview();
    });
    state.spreadsheetProgress = null;
...
analyticsContent?.addEventListener("click", (event) => {
  ...
  const detail = block?.querySelector("[data-other-brands-detail]");
  if (!detail) return;
  const willOpen = detail.hidden;
  detail.hidden = !willOpen;
  block.querySelectorAll("[data-expand-other-brands]").forEach((node) => {
    node.setAttribute("aria-expanded", String(willOpen));
  });
  if (willOpen) detail.scrollIntoView({ behavior: "smooth", block: "nearest" });
});
...
document.querySelector("#ads-bulk-price")?.addEventListener("click", async (event) => {
  if (event.target.closest("[data-clear-selected-ads]")) {
    state.adsSelectedIds.clear();
    renderAds();
    return;
  }
  ...
  const mode = root.querySelector("[data-bulk-price-mode]")?.value || "fixed";
  const value = Number(root.querySelector("[data-bulk-price-value]")?.value || 0);
